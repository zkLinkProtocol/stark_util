...
impl<'a, 'de, DE: Decoder> Deserializer<'de> for SerdeDecoder<'a, DE> {
        ...
        where V: Visitor<'de>
    {
        ...
        if variant.is_some() { visitor.visit_some(self) } else { visitor.visit_none() }
    }
        ...
        where V: Visitor<'de>
    {
        ...
        impl<'de, 'a, 'b: 'a, DE: Decoder + 'b> SeqAccess<'de> for Access<'a, 'b, DE> {
                ...
                where T: DeserializeSeed<'de>
            {
                if self.len > 0 {
                    self.len -= 1;
                    let value = DeserializeSeed::deserialize(seed, SerdeDecoder { de: self.deserializer.de })?;
                    Ok(Some(value))
                } else {
                    Ok(None)
                }
        ...
        visitor.visit_seq(Access { deserializer: &mut self, len })
    }
        ...
        where V: Visitor<'de>
    {
        ...
        impl<'de, 'a, 'b: 'a, DE: Decoder + 'b> MapAccess<'de> for Access<'a, 'b, DE> {
                ...
                where K: DeserializeSeed<'de>
            {
                if self.len > 0 {
                    self.len -= 1;
                    let key = DeserializeSeed::deserialize(seed, SerdeDecoder { de: self.deserializer.de })?;
                    Ok(Some(key))
                } else {
                    Ok(None)
                }
        ...
        visitor.visit_map(Access { deserializer: &mut self, len })
    }
